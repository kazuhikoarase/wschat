...
var wschat;
(function (wschat) {
    wschat.createChatClient = function (opts) {
        var ws = null;
        var chat = {
            user: null,
            users: {},
        ...
        var draggable = function (proxyFactory, $dragTarget, $dropTarget, canDrop, dropHandler, symbols) {
            var $proxy = null;
            var $symbol = null;
            var dragPoint = null;
        ...
        var getDateLabel = function (d) {
            ...
            var getShortLabel = function () {
                return messageFormat(chat.messages.SHORT_DATE_FORMAT, getMonthLabel(date.getMonth()), date.getDate(), getDayLabel(date.getDay()));
            };
            var getFullLabel = function () {
                return messageFormat(chat.messages.FULL_DATE_FORMAT, date.getFullYear(), getMonthLabel(date.getMonth()), date.getDate(), getDayLabel(date.getDay()));
            };
        ...
        var overwrap = function () {
            ...
            return function ($u1, $u2) {
                var off1 = $u1.offset();
                var off2 = $u2.offset();
                return overwrap(off1.left, off1.left + $u1.outerWidth(), off2.left, off2.left + $u2.outerWidth()) &&
                    overwrap(off1.top, off1.top + $u1.outerHeight(), off2.top, off2.top + $u2.outerHeight());
            };
        }();
        ...
        var putMessage = function (gid, message) {
            ...
            if (message.newMsg &&
                ...
                if (!isActive()) {
                    notify.start();
                    notificationManager.notify(message.nickname + ' ' +
                        getTimestampLabel(message.date), message.message);
                    notifySound();
                }
            }
            groupsUI.invalidate();
        ...
        var setSelectedView = function (selectedView) {
            ...
            $usersFrame.css('display', chat.selectedView == 'users' ? 'block' : 'none');
            $groupsFrame.css('display', chat.selectedView == 'groups' ? 'block' : 'none');
        ...
        var sendRequest = function (user) {
            var $editor = $('<input type="text"/>').
                ...
                val(messageFormat(chat.messages.CONTACT_ADD_REQUEST, user.nickname || user.uid));
            var $content = $('<div></div>').
                append($editor).
                append($('<br/>')).
                append(createButton(chat.messages.CANCEL).
        ...
        var searchContactHandler = function (event) {
            ...
            var $content = $('<div></div>').
                ...
                if (selectedUid == null) {
                    var dlg = createDialog();
                    dlg.showDialog(createDialogContent(dlg, chat.messages.SELECT_CONTACT, [chat.messages.OK]));
                }
                else {
                    sendRequest(users[selectedUid]);
                }
        ...
        var deleteContactHandler = function (event) {
            ...
            if (users.length == 0) {
                ...
                var dlg = createDialog();
                dlg.showDialog(createDialogContent(dlg, messageFormat(chat.messages.CONFIRM_DELETE_CONTACT, user.nickname || user.uid), [chat.messages.CANCEL, chat.messages.OK]).
        ...
        userUI.validate = function () {
            ...
            var $view = attachDnD(createUserView(), function (event) {
        ...
        var updateThreadMessage = function (gid, message) {
            ...
            var update = function ($cellsContent) {
                ...
                if (message.deleted) {
                    $cell.children('.wschat-thread-msg-body').append($('<span></span>').addClass('wschat-deleted').
                        text(chat.messages.DELETED));
                }
                if (!message.deleted && message.file) {
                    if (!message.file.deleted) {
                        $cell.children('.wschat-thread-msg-body').append($('<span></span>').addClass('wschat-attached-file').
                            css('cursor', 'default').
                        ...
                        $cell.children('.wschat-thread-msg-body').append($('<span></span>').addClass('wschat-deleted').
                            text(message.file.name)).append($('<span></span>').addClass('wschat-deleted').
                            css('margin-left', '4px').
                ...
                if (approveMsg) {
                    $cell.children('.wschat-thread-msg-body').append(createButton(chat.messages.APPROVE).on('click', function (event) {
        ...
        threadUsersUI.validate = function () {
            ...
            $.each(users, function (i, uid) {
                var factory = function () {
                    return createThreadUser(group, uid, users.length < 4 ? 'default' : 'small');
                };
                var $user = factory().
                    on('mousedown', function (event) {
                    event.preventDefault();
                ...
                draggable(factory, $user, $tabContent, function () {
        ...
        threadUI.validate = function () {
            ...
            $thread.append(function () {
                var $btn = $('<span></span>').
                    ...
                    on('click', function (event) {
                    var dlg = createDialog();
                    dlg.showDialog(createDialogContent(dlg, chat.messages.COMFIRM_EXIT_FROM_THIS_GROUP, [chat.messages.CANCEL, chat.messages.OK]).
                        on('close', function (event, button) {
                        if (button != chat.messages.OK) {
                            return;
                        }
        ...
        var validateThreadMessages = function () {
            ...
            while (chat.threadMessages.length > 0 && getTime() - start < 50) {
                updateThreadMessage(chat.threadMessagesGid, chat.threadMessages.shift());
            }
        };
        var layout = function () {
            $usersFrame.
