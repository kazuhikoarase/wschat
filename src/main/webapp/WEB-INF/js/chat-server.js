...
(function (wschat_1) {
    ...
    var messageFormat = function () {
        var args = java.lang.reflect.Array.newInstance(java.lang.Class.forName('java.lang.Object'), arguments.length - 1);
    ...
    var createChatService = function () {
        ...
        var toJavaOpts = function (opts) {
            ...
            $.each(opts, function (k, v) {
                javaOpts.put(new java.lang.String('' + k), new java.lang.String('' + v));
            });
            return javaOpts;
        };
        ...
        var toJsUserJSON = function (javaUser) {
            var user = JSON.parse(javaUser.getJsonData());
            user.uid = toJsString(javaUser.getUid());
            user.nickname = user.nickname || user.uid;
            user.contacts = [];
        ...
        var updateAvatar = function (uid, file) {
            ...
            var data = Packages.ws.util.Base64.toUrl(file.contentType, tmpfile.getCanonicalPath());
        ...
        var newGroup = function (users) {
            ...
            return toJsString(service.newGroup(javaUsers, JSON.stringify({})));
        };
        ...
        var addToGroup = function (uid, group, uidToAdd) {
        ...
        var removeFromGroup = function (uid, group, uidToRemove) {
            var oldUsers = getGroupUsers(group);
    ...
    var createChatEndpoint = function () {
        ...
        actions.newGroup = function (data) {
            ...
            if (users.length > 2) {
                ...
                sendSystemMessage(chatService.getGroup(chat.user.uid, data.gid), messageFormat(chat.messages.ADD_TO_GROUP, getNickname(chat.user.uid), nicknames));
            }
        ...
        actions.addToGroup = function (data) {
            ...
            if (isGroupMember(group)) {
                ...
                if (group != null) {
                    ...
                    sendSystemMessage(group, messageFormat(chat.messages.ADD_TO_GROUP, getNickname(chat.user.uid), getNickname(data.uid)));
                }
            }
        };
        actions.removeFromGroup = function (data) {
            ...
            if (isGroupMember(group)) {
                ...
                sendSystemMessage(oldGroup, messageFormat(chat.messages.REMOVE_FROM_GROUP, getNickname(chat.user.uid), getNickname(data.uid)));
            }
        };
        actions.exitFromGroup = function (data) {
            ...
            if (isGroupMember(group)) {
                var oldGroup = JSON.parse(JSON.stringify(group));
                group = chatService.removeFromGroup(chat.user.uid, group, chat.user.uid);
                data.group = group;
                send(data);
                $.each(oldGroup.users, function (uid, user) {
                    send({
                ...
                sendSystemMessage(oldGroup, messageFormat(chat.messages.EXIT_FROM_GROUP, getNickname(chat.user.uid)));
            }
        };
        actions.fetchGroups = function (data) {
            $.each(chatService.fetchGroups(chat.user.uid, data.opts), function (gid, group) {
                send({
                    action: 'group',
                    group: group
                });
        ...
        actions.fetchMessages = function (data) {
            $.each(chatService.fetchMessages(chat.user.uid, data.gid, data.opts), function (mid, message) {
                send({ action: 'message',
                    gid: data.gid,
                    message: message
                });
    ...
    wschat_1.createServer = createServer;
})(wschat || (wschat = {}));
